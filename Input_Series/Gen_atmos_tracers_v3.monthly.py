# Update 12/12/2021
# Writes all tracer input histories at monthly resolutions
# This relies on linear interpolation for CFC and SF6
...
trit_vienna = trit_atmos3.data.H3_Vienna

# Michel and Jurgens 2018, DOI: https://doi.org/10.3133/sir20185086
# Interpolated tritium across CONUS -- 1553-2012 at monthly resolution
# At monthly resol
trit_qd = pd.read_csv('./Tritium/TritiumInQuadrangeles_Jurgens2018.csv')
trit_mic = trit_qd.loc[:,['Date', 'Year', 'Month','39-37,110-105']] # these are Crested Butte degree coordinates
trit_mic.columns = ['Date', 'Year', 'Month','H3']
trit_mic.index = pd.to_datetime(trit_mic['Date'])
trit_mic = trit_mic['H3'][:713] # drop the last 3 or so nan values
...
# Using ottowa to extend forward

# Make my own correlation against Ottowa
trit_df = pd.merge(trit_ottowa, trit_mic, how='outer', left_index=True, right_index=True)
# Sort the month
...
# extend Michel tritium forward using the above relationship
trit_df['H3_ext'] = trit_df['H3'].copy()
for i in trit_df.index[trit_df['H3'].isna()]:
    ott = np.log10(trit_df.loc[i,'H3_Ottowa'])
    log_trit_ext = df_corr.loc[i.month,'slope']*ott + df_corr.loc[i.month,'intercept']
    trit_df.loc[i, 'H3_ext'] = 10**log_trit_ext
...
trit = trit_df['H3_ext']
trit.interpolate(method='linear', axis=0, limit=10, inplace=True, limit_direction='forward', limit_area=None)
background = trit.min()
...
# combine
# this at monthly resolution
trit_ext = pd.concat((back_df, trit))
# use interpolation to fill nans
trit_ext.interpolate(method='quadratic', inplace=True)
trit_ext.name = 'tritium'
...
# Linear Interpolation to monthly resolution
cfc_sf6_mth = cfc_sf6.resample('M').mean().interpolate(method='linear')
# Set the dat to 15 to match tritium
cfc_sf6_mth.index = cfc_sf6_mth.index.map(lambda x: x.replace(day=15))
...
tracer_atmos = get_atm_conc.concat_resampled_time_series(C_t_gas=cfc_sf6_mth, C_t_isotopes=trit_ext, freq='M', hemisphere='NH')
...
tracer_atmos.rename(columns={'CFC11NH':'CFC11',
                              'CFC12NH':'CFC12',
                              'CFC113NH':'CFC113',
                              'SF6NH':'SF6'}, inplace=True)

tracer_atmos.index.name = 'Date'
...
# Save to files
#---------------------------------
tracer_atmos.to_csv('CB_tracer_atmos_monthly.csv')
